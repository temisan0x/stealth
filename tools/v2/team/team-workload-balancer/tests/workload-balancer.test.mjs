...
assert.strictEqual(actualAssignments.length, fixture.workItems.length, 'All work items should return an assignment result')

const unassignedItem = actualAssignments.find((item) => item.taskId === 'task-accounting-review')
assert.strictEqual(unassignedItem.status, 'unassigned', 'Tasks with no matching skills should remain unassigned')
assert.strictEqual(unassignedItem.reason, 'skill mismatch', 'The reason should explain why the task was not assigned')

const assignedLoads = actualAssignments.filter((item) => item.status === 'assigned').map((item) => item.assigneeId)
assert.deepStrictEqual(
  assignedLoads,
  ['bob', 'cara', 'cara', 'alice'],
