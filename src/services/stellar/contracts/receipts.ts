...
export function parseReceiptsError(
  code: number
...
export async function delivered(
  client: contract.Client, message_id: Buffer, payload_hash: Buffer, protocol_version: number, sender: string, recipient: string
): Promise<contract.Ok<Receipt> | contract.Err<{ message: string }>> {
  const tx = await (client as any).delivered({ message_id, payload_hash, protocol_version, sender, recipient });
  return tx.result;
}
